...
public class T07EqualsHashcodeTest {
	...
	public static void main(String[] args) {
		 ...
		 * hashCode()를 호출한다. 그래서 객체가 서로 같은지 여부를 결정하려면 두 메서드를 재정의 해야한다. HashSet,HashMap,
		 * Hashtable 에서 객체가 같은 지 여부는 데이터를 추가할 때 검사한다.
		 * 
		 * - equals() => 두 객체의 내용(값)이 같은지 비교하는 메서드. - hashCode() => 객체에 대한 해시코드값을 반환하는
		 * 메서드.
		 * 
		 * hash를 사용할때 데이터를 찾는 속도가 빠르다 hash는 충돌이 날 가능성이 있다.
		 *
		 * - equals()와 hashCode()에 관한 규칙 
		 * 
		 * 1. 두 객체가 같으면 반드시 같은 hashCode를 가져야한다. 
		 * 2. 두 객체가 같으면 equals()메서드를 호출했을 때 true값은 반환해야 한다. 
		 *   즉, 객체 a,b가 같다면a.equals(b)와 b.equals(a) 둘 다 true이여야 한다.
		 * 3. 두 객체의 hashCode가 같다고 해서 두 객체가 반드시 같은 객체는 아니다.
		 * 	  하지만 두 객체가 같으면 반드시 hashCode 같아야 한다. 
		 * 4. equals()를 override 하면 반드시 hashCode도 override 해야 한다. 
		 * 5. hashCode()는 기본적으로 Heap 메모리에 있는 각 객체에 대한 메모리 주소 매핑 정보를 기반으로 한 정수값을 반환한다. 
		 *    그러므로 클래스에서 hashCode() 메서드를 override하지 않으면 
		 *    절대로 두 객체가 같은 해시값을 반환하지 않는다.
		 * 
		 * - hashCode에서 사용하는 '해싱알고리즘' 에서 서로 다른 객체에 대하여 같은 hashCode 값을 만들어 낼 수 있다. 
	...
	System.out.println("add(p2) 성공 여부 : " + pSet.add(p3));
