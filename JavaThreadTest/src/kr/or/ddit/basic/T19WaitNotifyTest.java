...
public class T19WaitNotifyTest {
	/*
	 * wait() => 동기화 영역에서 락을 풀고 Wait - Set영역(공유객체별존재)
	 * 		으로 이동 시킨다.
	 * notify() 또는 notifyAll() => Wait-Set 영역에 있는 스레드를 깨워서
	 * 							     실행될 수 있도록 한다
	 * (notify()는 하나, notifyAll()은 전부를 깨운다)
	 * 
	 * => wait()과 notify, notifyAll()메서드는 동기화 영역에서만 실행
	 * 할 수 있고, Object 클래스에서 제공하는 메서드이다.
	 * 
	 * */
