...
public class T17LockAccountTest {
	 ...
	 * 				   Synchronized를 이용한 동기화 처리보다 부가적인
	 * 				      기능이 제공된다
	 * 				   ex) Fairness(공평하다/공정하다) 설정 등. => 가장 오래 기다린 스레드가
	 * 				     	가장 먼저 락을 획득하게
	 * ReentrantReadWriteLock : 
	 * 			Read 및 Write 락을 구분하여 사용 가능 함.
	 * 			여러 스레드가 동시에 read작업은 가능하지만, write 작업은
	 * 			단지 하나의 스레드만 가능함.
	 * 			=> write 보다 read 위주의 작업이 많이 발생하는 경우에 사용하면
	 * 			      유리하다
	 * */
...
class LockAccount {
	...
	// lock 갹체 생성 => 되도록이면 private final로 만든다.
	...
	public void deposit(int money) {
		// Lock 객체의 lock() 메소드가 동기화 시작이고, unlock()
		// 메서드가 동기화의 끝을 나타낸다
		// lock() 메서드로 동기화를 설정한 곳에서는 반드시 unlock()
		// 메서드로 해제해 주어야한다.
		lock.lock(); // 락설정(락을 획득하기 전까지 Bolck상태
		balance += money; // 동기화 처리부분
		lock.unlock(); // 락 해제
	}

	// 출근하는 메서드(출금 성공 : true, 출근 실패: false)
...
class BankThread2 extends Thread {
	...
	public void run() {
		...
		System.out.println("스레드안에서 result =" + result + ", balance = " + lAcc.getBalance());
