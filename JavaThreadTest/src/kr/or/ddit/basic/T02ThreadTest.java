...
public class T02ThreadTest {
	public static void main(String[] args) {
		...
		// 상속이라는 걸 이용해서 나도 Thread가 된다
		Thread th1 = new MyThread1();
//		th1.start();
		th1.run();
		// 방법 2: Runnable 인터페이스를 구현한 class의 인스턴스를 생성한 후
		// 이 인스턴스를 Tread객체의 인스턴스를 생성할 때 생성자의
		// 파라미터로 넘겨준다.
...
//		th2.start();
		...
		// 방법3: 익명클래스를 이용하는 방법
		// Runnable 인터페이스를 구현한 익명클래스를 Tread
		// 인스턴스를 생성할 때 매개변수로 넣어준다.
		Thread th3 = new Thread(new Runnable() {
			...
			public void run() {
				for (int i = 1; i < 200; i++) {
					...
					try {
						// Tread.sleep(시간)=>주어진 시간만큼 잠시 멈춘다.
						// 시간은 밀리세컨드 단위
						// (즉, 1000ms = 1초)
						Thread.sleep(100);
...
class MyThread1 extends Thread {
	...
	@Override // Thread에는 run이라는 메소드가 필요하다
	public void run() {
