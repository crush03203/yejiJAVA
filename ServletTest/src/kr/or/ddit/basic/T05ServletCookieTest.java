...
/*
   쿠키정보를 다루기 위한 예제
   (웹서버와 브라우저는 애플리케이션에서 사용중인 필요한 값을 쿠키를 이용하여
    상태정보를 유지함.)
   ...
   2. 동작방식
     ...
     - 쿠키저장단계 : 브라우져는 응답데이터에 포함된 쿠키를 쿠키저장소에 보관한다.
     - 쿠키전송단계 : 브라우져는 저장한 쿠키를 요청이 있을때마다 웹서버에 전송한다.
                 (삭제되기 전까지...)
                              웹서버는 브라우저가 전송한 쿠키를 사용해 필요한 작업을 수행한다.
*/
   // 이 두개만 해도 되는 이유? 브라우저를 통해서 요청하는 방법은 이 두가지가 끝!
   // 바디 존재x
   @Override
   protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {

//      setCookieExam(req, resp); // 쿠키 생성 예제
//      readCookiesExam(req,resp); // 쿠키 정보 읽기 예제
   ...
   private void readCookiesExam(HttpServletRequest req, HttpServletResponse resp) throws IOException {
   ...
   private void setCookieExam(HttpServletRequest req, HttpServletResponse resp) throws IOException {
      ...
      // 쿠키 생성하기
      // 사용불가문자(공백,[]()=,"/?@:;) => RFC2965
      // 쿠키값은 사용불가 문자를 제외한 나머지 출력가능한 아스키문자만 사용 가능함.
      // 이외의 값(예를 들면 한글)을 사용시에는
      ...
      // 쿠키 최대 지속시간 설정하기(초단위)
      // 지정하지 않으면 웹브라우져 종료시 쿠키정보는 삭제된다.
      userId.setMaxAge(60 * 60 * 24); // 하루동안 유지하고 싶다..
      // 자바스크립트를 통한 직접 접근 방지
      userId.setHttpOnly(true);

      name.setMaxAge(60 * 60 * 48); // 이틀동안 유지하고 싶다..
